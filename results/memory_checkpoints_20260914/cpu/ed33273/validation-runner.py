"""Record memory/checkpoint CPU checks, independent installed replay and analysis timings."""
...
def mark_unfinished():
    if manifest["status"] == "RUNNING":
        ...
        manifest["controller_error"] = "Validation exited before all stages completed; inspect stderr."
        save()
...
files = [
    ...
    "test_interpreter_provenance", "test_interpreter_value_mapping", "test_interpreter_memory_dependencies", "test_interpreter_matmul_checkpoints", "test_interpreter_shared_storage", "test_interpreter_trace_index", "test_ssa_application_lowering", "test_ssa_first_backend_lowering",
...
run("cpu", [sys.executable, "-m", "pytest", "-q", "--color=no", "-ra", "--tb=short", *[f"tests/{name}.py" for name in files], "-k", "not test_cpu_interpreter_matches_actual_triton_gpu", f"--basetemp={out / 'pytest-tmp'}", f"--junitxml={out / 'cpu.junit.xml'}"])
...
assert run("wheel-build", [uv, "build", "--offline", "--python", sys.executable, "--wheel", "--out-dir", out / "wheels", snapshot], cwd=out) == 0
...
installed_venv = Path("/home/xxl/.venvs") / ("nine-memory-" + expected[:7])
assert not installed_venv.exists()
assert run("wheel-venv", [uv, "venv", "--python", sys.executable, installed_venv], cwd=out) == 0
installed_python = installed_venv / "bin/python"
assert run("wheel-dependencies", [uv, "pip", "install", "--offline", "--python", installed_python, "numpy=="+manifest["packages"]["numpy"], "sympy=="+manifest["packages"]["sympy"]], cwd=out) == 0
assert run("wheel-install", [uv, "pip", "install", "--offline", "--python", installed_python, "--no-deps", wheel], cwd=out) == 0
...
mapped_cases = []
for failure in sorted((out / "pytest-tmp").rglob("failure.json")):
    data = json.loads(failure.read_text())
    name = (data.get("report") or {}).get("first_bad_pass", "") or ""
    if name.startswith("diagnostic.mapped_"):
        mapped_cases.append(failure.parent)
assert len(mapped_cases) == 4, mapped_cases
manifest["mapped_replays"] = []
for index, directory in enumerate(mapped_cases):
    data = json.loads((directory / "failure.json").read_text())
    run(f"installed-mapped-{index}", [installed_python, "-I", directory / "replay.py"], cwd=out)
    manifest["mapped_replays"].append({"directory": str(directory.relative_to(out)), "localization": data["report"]["localization"], "dependencies": data["report"]["dependency_slice"]})
legacy = repo / "results/pass_failure_workflow_20260908/9c5ffba/pipeline-replays"
for directory in sorted(legacy.iterdir()):
    if (directory / "replay.py").is_file():
        run("legacy-" + directory.name, [installed_python, "-I", directory / "replay.py"], cwd=out)
new_cases = []
for failure in sorted((out / "pytest-tmp").rglob("failure.json")):
    directory = failure.parent
    test_name = directory.relative_to(out / "pytest-tmp").parts[0]
    if test_name.startswith(("test_default_matmul_pass_loc", "test_shared_storage_failure_re")):
        new_cases.append(directory)
assert len(new_cases) == 21, new_cases
manifest["checkpoint_and_memory_replays"] = []
for index, directory in enumerate(new_cases):
    metadata = json.loads((directory / "failure.json").read_text())
    code = run(f"installed-checkpoint-memory-{index}", [installed_python, "-I", directory / "replay.py"], cwd=out)
    manifest["checkpoint_and_memory_replays"].append({"directory": str(directory.relative_to(out)), "exit_code": code, "report": metadata["report"]})
for index in range(4):
    directory = repo / "results/value_mapping_20260913/adf658e/mapped-cases" / str(index)
    run(f"legacy-mapped-{index}", [installed_python, "-I", directory / "replay.py"], cwd=out)
run("analysis-benchmark", [sys.executable, Path(__file__).parent / "benchmark_nine_localization_20260914.py", repo, out / "analysis-benchmark", expected], environ=dict(env, PYTHONPATH=str(repo / "src")+os.pathsep+str(repo)))
