...
def evaluate(expression, symbols):
    ...
    try:
        evaluator = _compiled_expression(expression)
    except TypeError:
...
def _compiled_expression(expression):
    ...
    if expression.op in BINARY and len(expression.operands) == 2:
        left, right = map(_compiled_expression, expression.operands)
        return partial(_binary, BINARY[expression.op], left, right)
    if expression.op in UNARY and len(expression.operands) == 1:
        return partial(_unary, UNARY[expression.op], _compiled_expression(expression.operands[0]))
    return partial(_evaluate, expression)
