...
class _ExpressionIdentity:
    ...
    def __eq__(self, other):
        return isinstance(other, _ExpressionIdentity) and self.expression is other.expression
...
def _compiled_expression(identity):
    ...
    if expression.op in BINARY and len(expression.operands) == 2:
        left, right = (
            _compiled_expression(_ExpressionIdentity(value)) for value in expression.operands
        )
        return partial(_binary, BINARY[expression.op], left, right)
    if expression.op in UNARY and len(expression.operands) == 1:
        return partial(_unary, UNARY[expression.op], _compiled_expression(_ExpressionIdentity(expression.operands[0])))
    return partial(_evaluate, expression)
