...
def workloads():
    ...
    for shape, layout in zip(protocol['target_shapes'], protocol['target_layouts']):
        ...
        yield 'identity_'+'x'.join(map(str,shape))+'_'+layout, True, x, x+x, program, {}
    for name, shape, arrange, apply in [('vector', (4096,), vector_layout, affine), ('softmax',(17,257),row_layout,softmax)]:
        ...
        yield name, False, x, expected, kernel.program, {'tensors':kernel.tensors,'symbols':kernel.meta}
...
with args.output.open('x') as output:
    ...
    save()
    for name,target,x,expected,program,options in workloads():
        out = np.empty_like(x)
        original = x.copy()
        for trace in (False,True):
            def run():
                return interpret_program(program,{'x':x,'out':out},trace=trace,**options)
            result=run();np.testing.assert_allclose(result.outputs['out'],expected,rtol=1e-5,atol=1e-5)
            ...
            for mode in ('warm','cold'):
                ...
                np.testing.assert_allclose(result.outputs['out'],expected,rtol=1e-5,atol=1e-5)
