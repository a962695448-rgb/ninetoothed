...
def _left_literal(function, left, right, symbols):
    return function(left, right(symbols))


def _right_literal(function, left, right, symbols):
    return function(left(symbols), right)


def _literal_application(function, values, symbols):
...
def _build_expression(expression):
    ...
    if expression.op in BINARY and len(expression.operands) == 2:
        ...
        # Literal values are already present. Operators must still execute at
        # runtime so warning policies, custom numeric objects and errors stay live.
        if left.op == "constant" and right.op == "constant":
            return partial(_literal_application, function, (left.value, right.value))

        if left.op == "constant":
            return partial(
                _left_literal, function, left.value, _build_expression(right)
            )

        if right.op == "constant":
            return partial(
                _right_literal, function, _build_expression(left), right.value
            )

        return partial(
            _binary, function, _build_expression(left), _build_expression(right)
        )
    ...
    if expression.op in UNARY and len(expression.operands) == 1:
        ...
        if operand.op == "constant":
            return partial(_literal_application, function, (operand.value,))

        return partial(_unary, function, _build_expression(operand))
    return partial(_evaluate, expression)
