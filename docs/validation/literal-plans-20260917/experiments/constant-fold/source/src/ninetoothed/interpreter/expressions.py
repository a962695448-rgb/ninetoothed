...
def _fold_constants(function, *operands):
    # Only exact Python integers/booleans and known pure operators are safe to
    # evaluate here. NumPy warnings, floats, custom objects and errors stay live.
    if not any(function is allowed for allowed in _FOLDABLE):
        return None

    values = []

    for operand in operands:
        if not isinstance(operand, partial) or operand.func is not _constant:
            return None

        value = operand.args[0]

        if (
            not (type(value) is int or type(value) is bool)
            or int.bit_length(value) > 64
        ):
            return None
    ...
    if (type(value) is int or type(value) is bool) and int.bit_length(value) <= 64:
        return partial(_constant, value)
...
def _build_expression(expression):
    ...
    if expression.op in BINARY and len(expression.operands) == 2:
        ...
        function = BINARY[expression.op]
        folded = _fold_constants(function, left, right)

        return folded if folded is not None else partial(_binary, function, left, right)

    if expression.op in UNARY and len(expression.operands) == 1:
        function = UNARY[expression.op]
        operand = _build_expression(expression.operands[0])
        folded = _fold_constants(function, operand)

        return folded if folded is not None else partial(_unary, function, operand)
    return partial(_evaluate, expression)
