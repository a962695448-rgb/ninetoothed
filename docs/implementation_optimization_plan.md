...
本轮补充实际读写的相对字节区间、同program最后写入依赖、重叠视图的独立复制与回放、矩阵乘输入/乘积/累加前缀检查点，以及执行记录索引。掩码、截断记录、未知handler、跨program顺序和重叠写入保持明确边界。新增功能正在进行完整CPU、独立wheel、新旧故障包、文档和性能回归；随后以冻结源码集中验证GPU。本轮的新GPU结果尚未取得，不能沿用下方旧版本的通过数。接口见[CPU解释器说明](source/cpu_interpreter.rst)。
