...
## 2026-09-14 内存依赖与矩阵乘中间检查点

最终源码`ed33273`补充实际读写的相对字节区间、同program最后写入依赖、重叠视图的独立复制与回放、矩阵乘输入/乘积/累加前缀检查点，以及执行记录索引和纯值快速路径。460项CPU回归通过，独立wheel、新旧故障包、严格文档与性能对照通过；同源码A100完整835通过、2项双GPU条件跳过。掩码、截断记录、未知handler、跨program顺序和重叠写入保持明确边界。详细结果、性能负例及复现入口见[实施与验证报告](memory_checkpoints_20260914.md)，不与下方旧版本结果累加。

## 2026-09-13 结果对应与依赖定位

源码`adf658e`完成显式结果对应、真实linalg分解的完整输出比较、执行依赖追踪及新旧故障包兼容回放。383项选定CPU回归通过，其中包含55项新增检查；15项GPU取消选择，4组新包与4组历史包独立安装回放通过。后续`c180e2`补齐TileLang依赖，并在A100-SXM4-80GB完成758通过、2项双GPU条件跳过；详见[A100验收](a100_value_mapping_validation_20260913.md)。旧3fc27e0的A100703/2仍按旧源码保存。用户验收前不提交PR。

## 2026-09-09 完整回归收尾

源码`3fc27e0`已在真实A100完成703项通过、2项多GPU跳过的完整套件，退出0；同卡15项GPU差分包含在703中。原中断和90秒单例诊断均保留；最新环境、源码和原始日志见[完整验收记录](a100_complete_20260909.md)。代码与原容差没有为取得通过而修改。当前进入用户验收阶段，上游PR与正式提交尚未执行。

## 2026-09-09 A100 回归状态

源码 `3fc27e0` 已完成真实A100上的15项GPU差分和独立CUDA dot。空缓存完整回归在705项中的84项通过、2项跳过后中断，原退出码2与日志保留。下一步先恢复环境并核对有效缓存，再保持原测试集合和容差进行完整回归；不将部分通过累计成完整验收。见[本轮验证记录](a100_validation_20260909.md)。上游PR仍待用户验收。
