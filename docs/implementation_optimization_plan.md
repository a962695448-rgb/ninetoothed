...
源码`adf658e`完成显式结果对应、真实linalg分解的完整输出比较、执行依赖追踪及新旧故障包兼容回放。383项选定CPU回归通过，其中包含55项新增检查；15项GPU取消选择，4组新包与4组历史包独立安装回放通过。严格文档构建通过，见[实施与验收报告](value_mapping_20260913.md)。本轮没有GPU运行；旧3fc27e0的A100703/2仍按旧源码保存。下一步用冻结版本集中复验A100，用户验收前不提交PR。
