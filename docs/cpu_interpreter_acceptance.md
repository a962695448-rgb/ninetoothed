...
## 2026-09-13 结果对应定位的 CPU 验收

源码`adf658e`已通过383项选定CPU回归，包含55项新增定位检查；15项GPU测试被取消选择。新增显式结果对应、真实默认分解的标量输出比较、执行依赖解释和新旧故障独立回放，见[本轮报告](value_mapping_20260913.md)。这是新的CPU验证，下面3fc27e0的A100703/2属于此前版本，不能移用于新提交。新版本A100测试包已经准备，尚未执行；上游PR仍待用户验收。

## 2026-09-09 最新完整A100回归通过

源码`3fc27e0`已完成真实A100完整套件：**703 passed、2 skipped、929.31秒、退出0**，其中包含15项真实GPU差分。两项跳过均要求多GPU。96份原始文件下载并逐字核验，全库行覆盖率为9560/11032（86.66%）。见[最终验收报告](a100_complete_20260909.md)和[原始证据](../results/a100_complete_20260909/README.md)。同源码独立CUDA dot的此前A100记录保留原设备来源。复杂重构定位边界及上游PR待用户验收的状态不变。

下面的中断描述保留对应历史尝试，不再代表最新完整回归状态。

## 2026-09-09 A100 差分通过，完整回归待补

最新实际 A100 验证源码 `3fc27e0` 的15项GPU差分与独立CUDA dot通过。完整套件收集705项，但在执行窗口结束前中断：84 passed、2 skipped，pytest退出2；**不能标为完整回归通过**。原日志、中断状态及后续步骤见[验证说明](a100_validation_20260909.md)与[原始证据](../results/a100_validation_20260909/README.md)。旧 `5920018` 的完整682/2保持原范围，不迁移为新提交的结果。

## 2026-09-08 阶段定位与自动失败导出

当前功能源码 `9c5ffbad6d75e8b7f083270f08dc4009a8fe4424` 已完成 **328 项选定 CPU 回归（15 项 GPU 取消选择）**、普通 wheel 隔离回放、4 组真实默认 pass 管线自动导出与4组修复对照，以及独立文档环境的严格 Sphinx 构建。新增能力、定位依据和边界见[技术说明](pass_failure_workflow_20260908.md)，原始日志见[证据索引](../results/pass_failure_workflow_20260908/README.md)。复杂重构没有可信对应关系时仍只报告候选范围，不声称唯一根因。

本轮源码只改差分调试与失败材料模块，没有启动付费 GPU。下面的 A100 682 项结果对应 `5920018`，不能改记为本轮源码的实测。上游 PR 与正式提交仍待用户验收。
